...
// Answer begins here
func split(originalPlacer: Placer, _ splitPlacer: Placer) -> (Painter, Int) -> Painter {
    var splitInternal: (Painter, Int) -> Painter = { (_,_) in return wave }
    splitInternal = { (painter, n) in
        if n == 0 {
            return painter
        } else {
