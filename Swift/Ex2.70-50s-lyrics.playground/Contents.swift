...
let encodedLyrics = encode(lyrics, tree: lyricTree)
print("\(encodedLyrics) requires \(encodedLyrics.count) bits of information")
...
for lyric in Array(Set(lyrics)) {
    print("\(lyric):\(encode([lyric],tree: lyricTree))")
}
