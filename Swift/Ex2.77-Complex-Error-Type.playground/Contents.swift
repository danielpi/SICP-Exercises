...
func makeRationalNumber(_ n: Int, _ d: Int) -> Tagged<Pair> {
...
func installRectangularPackage() {
    ...
    func makeFromRealImag(_ x: Double, _ y: Double) -> Tagged<Pair> {
        return tag(Pair(car: x, cdr: y))
    }
    func makeFromMagAng(_ r: Double, _ A: Double) -> Tagged<Pair> {
...
func installComplexPackage() {
    ...
    // Internal Procedures
    func addComplex(_ z1: Pair, _ z2: Pair) -> Tagged<Pair> {
        return makeFromRealImag(realPart(z1) + realPart(z2), y: imagPart(z1) + imagPart(z2))
    }
    
    func subComplex(_ z1: Pair, _ z2: Pair) -> Tagged<Pair> {
        return makeFromRealImag(realPart(z1) - realPart(z2), y: imagPart(z1) - imagPart(z2))
    }
    
    func mulComplex(_ z1: Pair, _ z2:Pair) -> Tagged<Pair> {
        return makeFromMagAng(magnitude(z1) * magnitude(z2), A: angle(z1) + angle(z2))
    }
    
    func divComplex(_ z1: Pair, _ z2: Pair) -> Tagged<Pair> {
        return makeFromMagAng(magnitude(z1) / magnitude(z2), A: angle(z1) - angle(z2))
    }
