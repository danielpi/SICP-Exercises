...
(define (frame-outline frame)
  (let ((p1 (origin-frame frame))
        ...
        (p4 (add-vect (add-vect (origin-frame frame) (edge1-frame frame)) (edge2-frame frame))))
    ((segments->painter (list (make-segment p1 p2)
                              (make-segment p2 p4)
                              (make-segment p4 p3)
                              (make-segment p3 p1))) frame)))
...
(define (frame-cross frame)
  (let ((p1 (origin-frame frame))
    ...
    ((segments->painter (list (make-segment p1 p4)
                              (make-segment p2 p3))) frame)))
(frame-cross  (make-frame (make-vect 0 0) (make-vect 10 0) (make-vect 0 10)))
 
; c. The painter that draws a diamond shape by connecting the midpoints of the sides of the frame.
(define (midpoint-segment segment)
  (add-vect (start-segment segment)
            (scale-segment 0.5 segment)))
(define (frame-diamond frame)
  (let ((s1 (make-segment (origin-frame frame) (edge1-frame frame)))
        (s2 (add-vect (origin-frame frame) (edge1-frame frame)))
        (s3 (add-vect (origin-frame frame) (edge2-frame frame)))
        (s4 (add-vect (add-vect (origin-frame frame) (edge1-frame frame)) (edge2-frame frame))))
    ((segments->painter (list (make-segment p1 p4)
                              (make-segment p2 p3))) frame)))
