...
; of a tree. Each node of the tree holds one element of the set, canned the "entry" at that node,
...
; Figure 2.16 shows some trees that represent the set {1,3,5,7,9,11}. The same set may be represented 
; by a tree in a number of different ways. The only thing we require for a valid representation is
; that all elements in the left subtree be smaller than the node entry and that all elements in the
; right subtree be larger.

; The advantage of the tree representation is this: Suppose we want to check whether a number x
; is contained in a set. We begin by comparing x with the entry in the top node. If x is less than
; this, we know that we need only search the left subtree; if x is greater, we need only search the
; right subtree. Now, if the tree is "balanced", each of these subtrees will be about half the size
; of the original. Thus, in one step we have reduced the problem os searching a tree of size n to 
; searching a tree of size n/2. Since the size of the tree is halved at each step, we should expect
; that the number of steps needed to search a tree of size n grows as O(log n). For large sets,
; this will be a significant speedup over the previous representations.

; We can represent trees by using lists. Each node will be a list of three items: the entry at the 
; node, the left subtree, and the right subtree. A left or a right subtree of the empty list will
...
(define (element-of-set?3 x set)
  (cond ((null? set) false)
        ((= (entry set) x) true)
        ((> (entry set) x) (element-of-set?3 x (left-branch set)))
        ((< (entry set) x) (element-of-set?3 x (right-branch set)))))
(define set3 (make-tree 2 (make-tree 1 '() '()) (make-tree 3 '() '())))
(display "element-of-set?3")(newline)
(element-of-set?3 1 set3)
(element-of-set?3 2 set3)
(element-of-set?3 3 set3)
(element-of-set?3 4 set3)

; Adjoining an item to a set is implemented similary and also requires O(log n) steps. To adjoin 
; an item x, we compare x with the node entry to determine whether x should be added to the right
; or to the left branch together with the original entry and the other branch. If x is equal to 
; the entry, we just return the node. If we are asked to adjoin x to an empty tree, we generate a
; tree that has x as the entry and empty right and left branches. Here is the procedure:

(define (adjoin-set3 x set)
  (cond ((null? set) (make-tree x '() '()))
        ((= (entry set) x) set)
        ((> (entry set) x) (make-tree (entry set) (adjoin-set3 x (left-branch set)) (right-branch set)))
        ((< (entry set) x) (make-tree (entry set) (left-branch set) (adjoin-set3 x (right-branch set))))))

(display "adjoin-set3")(newline)
set3
(adjoin-set3 4 set3)

; The above claim that searching the tree can be performed in a logarithic number of steps rests 
; on the assumption that the tree is "balanced" i.e., that the left and the right subtree of every 
; tree have approximately the same number of elements, so that each subtree contains about half 
; the elements of its parent. But how can we be certain that the trees we construct will be balanced?
; Even if we start with a balanced tree, adding elements with adjoin-set may produce an unbalanced 
; result. Since the position of a newly adjoined element depends on how the element compares with
; the items already in the set, we can expect that if we add elements "randomly" the tree will tend 
; to be balanced on the average. But this is not a guarantee. For example, if we start with an
; empty set and adjoin numbers 1 through 7 in sequence we end up with the highly unbalanced tree
; shown in Figure 2.17. In this tree all the left subtrees are empty, so it has no advantage over 
; a simple ordered list. One way to solve this problem is to define an operation that transforms an
; arbitrary tree into a balanced tree with the same elements. Then we can perform this transformation 
; after every few adjoin-set operations to keep our set in balance. There are also other ways to
; solve this problem, most of which involve designing new data structures for which searching and 
; insertion both can be done in O(log n) steps.


; Sets and information retrevial
; We have examined options for using lists to represent sets and have seen how the choice of
; representation for a data object can have a large impact on the performance of the programs that
; use the data. Another reason for concentrating on sets is that the techniques discussed here
...
; files for a company of the transactions in an accounting system. A typical data-management system
; spends a large amount of time accessing or modifying the data in the records and therefore
; requires an efficient method for accessing records. This is donw by identifying a part of each
; record to serve as an identifying key. A key can be anything that uniquely identifies a record.
; For a personell file, it might be an employee's ID number. For an accounting system, it might
; be a transaction number. Whatever the key is, when we define the record as a data structure we
; should include a key selector procedure that retrieves the key associated with a given record.

; Now we represent the data base as a set of records. To locate the record with a given key we
; use a procedure lookup, which takes as arguments a key and a data base and which returns the
; record that has that key, or false if there is no such record. lookup is implemented in almost
; the same was as element-of-set?. For example, if the set of records is implemented as an
...
; retrevial systems in which records have to be "randomly accessed" are typically implemented
; by a tree-based method, such as the binary-tree representation discussed previously. In designing
; such a system the methodology of data abstraction can be a great help. The designer can create an
; initial implementation using a simple, straightforward representation such as unordered lists. This
; will be unsuitable for the eventual system, but it can be useful in providing a "quick and dirty"
; data base with which to test the rest of the system. Later on, the data representation can be
; modified to be more sophisticated. If the data base is accessed in terms of abstract selectors
; and constructors, this change in represendtaion will not require any changed to the rest of the
; system.
